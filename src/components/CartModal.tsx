        ...
        >
          ...
          <div className="px-8 py-6 border-b border-[#363330] flex justify-between items-center bg-[#1C1917]/80 backdrop-blur-md sticky top-0 z-10">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-[14px] uppercase font-bold tracking-[0.3em] text-[#F9F8F6]">Your Collection</h2>
                  ...
                  <div className="flex-1 flex flex-col justify-between py-1">
                    ...
                    <div className="flex justify-between items-end">
                      <div className="flex items-center space-x-4 bg-[#23211F] border border-[#363330] rounded-lg px-2 py-1">
                        ...
                        <span className="text-[12px] font-bold text-[#F9F8F6] w-4 text-center">{item.quantity}</span>
                        <button
                          onClick={() => updateQuantity(item.id, item.quantity + 1)}
                          className="text-stone-500 hover:text-white transition-colors"
                        >
                      ...
                      <p className="text-[13px] font-bold text-[#F9F8F6]">${(Number(item.price) * item.quantity).toFixed(2)}</p>
                    </div>
                  </div>
                </motion.div>
              ))
          ...
          {items.length > 0 && (
            <div className="p-8 border-t border-[#363330] bg-[#1C1917] space-y-6">
              <div className="flex justify-between items-center">
                <span className="text-stone-500 text-[11px] uppercase font-bold tracking-[0.2em]">Subtotal</span>
                <span className="text-[#F9F8F6] text-xl font-bold font-serif italic">${totalPrice.toFixed(2)}</span>
              </div>
