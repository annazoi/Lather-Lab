        ...
        <div className="max-w-md w-full px-6 text-center space-y-8 animate-in fade-in zoom-in duration-700">
          ...
          <div className="space-y-4">
            ...
            <p className="text-stone-500 text-sm leading-relaxed tracking-wide uppercase">
              Thank you for supporting artisanal craftsmanship. <br /> A confirmation has been sent to {user?.email}.
            </p>
          </div>
          <Link
            href="/collections"
          ...
          <div className="lg:col-span-7 space-y-12">
            <section className="space-y-8">
              <div className="border-b border-stone-200 pb-6">
                ...
                <p className="text-stone-400 text-xs uppercase tracking-widest mt-2">Where shall we send your botanicals?</p>
              </div>

              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase font-bold tracking-widest text-stone-500 ml-1">Recipient Name</label>
                    <input
                      required
                      type="text"
                      name="shippingName"
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase font-bold tracking-widest text-stone-500 ml-1">Delivery Address</label>
                    <input
                      required
                      type="text"
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase font-bold tracking-widest text-stone-500 ml-1">City</label>
                      <input
                        required
                        type="text"
                        name="shippingCity"
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase font-bold tracking-widest text-stone-500 ml-1">Contact Phone</label>
                      <input
                        required
                        type="tel"
                        name="shippingPhone"
                ...
                <div className="pt-8">
                  ...
                  >
                    {isSubmitting ? (
                      ...
                      <>
                        <CreditCard size={16} />
                        <span>Confirm Ritual & Purchase</span>
                      </>
                    )}
                  </button>
                </div>
            ...
            <section className="grid grid-cols-1 sm:grid-cols-3 gap-8 py-10 border-t border-stone-100">
              {[
                ...
                <div key={idx} className="flex flex-col items-center sm:items-start text-center sm:text-left space-y-2">
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-widest text-[#1C1917]">{prop.title}</p>
                  <p className="text-[9px] uppercase tracking-widest text-stone-400 leading-normal">{prop.sub}</p>
                </div>
              ))}
            </section>
          </div>
            ...
            <div className="bg-white border border-stone-200 rounded-sm p-8 lg:p-10 sticky top-32">
                  ...
                  <div key={item.id} className="flex space-x-6">
                    ...
                    <div className="flex-1 flex flex-col justify-between py-1">
                      <div>
                        ...
                        <p className="text-[9px] uppercase tracking-widest text-stone-400 mt-1">{item.category}</p>
                      </div>
                      <div className="flex justify-between items-end">
                        <span className="text-[10px] text-stone-400 uppercase font-bold tracking-widest">Qty: {item.quantity}</span>
                        <span className="text-[13px] font-bold text-[#1C1917]">${(Number(item.price) * item.quantity).toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-4 border-t border-stone-100 pt-8">
                ...
                <div className="flex justify-between items-center pt-4 border-t border-stone-100">
                  <span className="text-[12px] uppercase font-bold tracking-[0.2em] text-[#1C1917]">Total Ritual</span>
                  <span className="text-2xl font-serif italic font-bold text-[#1C1917]">${totalPrice.toFixed(2)}</span>
                </div>
              </div>
